...
const LLM_DEFAULT_MODELS = {
  // Pinned rather than `gemini-flash-latest`: an alias moves under you, and a
  // corpus generated by a different model than the README claims is not
  // reproducible. 2.5-flash was the original choice and now 404s for new keys.
...
export function toAppConfig(env: RawEnv, baseDir: string): AppConfig {
  ...
  return {
    ...
    // Resolved against the repo root rather than the cwd: the API starts from
    // apps/api and the CLI scripts from the root, and a cwd-relative path
    // silently produced two different storage directories.
    storageDir: resolve(baseDir, env.CV_STORAGE_DIR),
    llm: {
